...
buscar.addEventListener("click", (e) => {
    ...
    alert("Introduzca el nombre o numero de un pokemon")
      ...
      let pokemon = `<div type="button" class="resultado btn btn-lg btn-gradient btn-light imagenes col" data-bs-toggle="popover" data-bs-title="${pokemons[i].name} - ${pokemons[i].number}" data-bs-content="Peso = ${pokemons[i].weight} Kg || Altura : ${pokemons[i].height} cm || Debilidades : ${Debil(i)} || Habilidades : [${pokemons[i].abilities}] ">  
      <img src="${pokemons[i].ThumbnailImage}"> 
      <p class="numero"> N°.${pokemons[i].number}</p>
      <h3 class="nombre"> ${pokemons[i].name}</h3>
      ${Elemento(i)}  
